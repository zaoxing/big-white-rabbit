...
//   1. BWR_PYTHON_OVERRIDE env var — dev escape hatch.
...
//      apps/bwr-mac/Scripts/build.sh copies verbatim into the Swift .app.
...
//     so `python -m bwr.cli` resolves both the bwr package (shipped as a
//     pure source tree in Resources/bwr/, matching today's Python build)
...
struct PythonRuntime {
    ...
    static func resolve() throws -> PythonRuntime {
        ...
        if let override = env["BWR_PYTHON_OVERRIDE"], !override.isEmpty {
    ...
    func makeEnvironment() -> [String: String] {
        ...
        // starts; setting it inside bwr.cli is too late for malloc init.
