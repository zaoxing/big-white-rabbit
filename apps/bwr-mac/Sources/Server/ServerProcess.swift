// Lifecycle owner for the `bwr serve` child process.
//
// State machine
...
//
// Dev override: BWR_DEV_SERVER_SCRIPT spawns <python> <script> --port …
...
final class ServerProcess: @unchecked Sendable {
    ...
    static let stateDidChangeNotification = Notification.Name("BWRServerProcessStateDidChange")
    static let portConflictNotification   = Notification.Name("BWRServerPortConflict")
    ...
    func start() throws -> StartResult {
        ...
        let nextHost = env["BWR_HOST"].flatMap { $0.isEmpty ? nil : $0 }
            ?? saved.bindAddress ?? bindAddress
        let nextPort = env["BWR_PORT"].flatMap(Int.init) ?? saved.port ?? port
        ...
        if resolver.isPortInUseSync() {
            let conflict = PortConflict(
                ...
                isBWR: resolver.isBWROnPortSync()
            )
            update(.failed(message: "Port \(port) in use" +
                           (conflict.isBWR ? " (BigWhiteRabbit server already running)" : "")))
    ...
    private func tryAutoRestart(reason: String) {
        ...
        NSLog(
            "BigWhiteRabbit: auto-restart \(attempt)/\(autoRestartBudget.maxAttempts) " +
    ...
    private func logHealthProbeFailure(
        ...
        NSLog(
            "BigWhiteRabbit: health probe failed url=\(result.url) latency_ms=\(result.latencyMs) status=\(status) error=\(error) failures=\(failures) suppressed_by_recent_status=\(suppressed)"
        )
    }
