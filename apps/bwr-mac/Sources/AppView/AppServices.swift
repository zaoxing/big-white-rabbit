...
final class AppServices: NSObject {
    ...
    init(config: AppConfig = .default, server: ServerProcess? = nil) {
        self.config = config
        self.client = BWRClient(host: config.host, port: config.port, apiKey: config.apiKey)
    ...
    ///     `BWR_BASE_PATH` (env + bootstrap file), and
    ...
    private func migrateBasePath(to newPath: String) throws {
        ...
        // When the user resets to the `~/.bwr` default, clear every
        ...
        do {
            ...
            NSLog("BigWhiteRabbit: relocateOrphanPaths failed: %@", String(describing: error))
        }
    }
    ...
    nonisolated static func relocateOrphanPaths(in url: URL, oldBase: String, newBase: String) throws {
        NSLog("BigWhiteRabbit: relocateOrphanPaths in=%@ old=%@ new=%@",
              url.path, oldBase, newBase)
        guard FileManager.default.fileExists(atPath: url.path) else {
            NSLog("BigWhiteRabbit: relocateOrphanPaths skipped — file does not exist")
            return
        }
        let data = try Data(contentsOf: url)
        guard var json = try JSONSerialization.jsonObject(with: data) as? [String: Any]
        else {
            NSLog("BigWhiteRabbit: relocateOrphanPaths skipped — root is not an object")
            return
        }
        ...
        NSLog("BigWhiteRabbit: relocateOrphanPaths wrote %d bytes", out.count)
    }
