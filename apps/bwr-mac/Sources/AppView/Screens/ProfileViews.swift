...
// on the Server screen. The shapes mirror bwr-screens.jsx:626-1191:
//
...
/// Per-scope visual treatment. Lifted from bwr-screens.jsx:878-884
/// (SCOPE_META) so the chip dots and badges read the same as the canvas.
enum ProfileScopeMeta {
    static func color(_ scope: ProfileScope, theme: BWRTheme) -> Color {
        switch scope {
...
struct ProfileGroup: View {
    ...
    /// bwr.ai via `POST /api/presets/refresh`.
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            Text(label)
                .font(.bwrText(10.5, weight: .heavy))
                .kerning(0.7)
                .textCase(.uppercase)
            ...
            if let onRefresh {
                ...
                .help(String(localized: "profile.group.refresh.help",
                             defaultValue: "Refresh presets from bwr.ai",
    ...
    /// (`bwr/model_profiles.py:validate_profile_name`). Pre-checking
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(titleText)
                    .font(.bwrText(13.5, weight: .semibold))
            ...
            case .working(let basedOn):
                ...
                if let onSaveAsNew {
                    Button(String(localized: "profile.banner.action.save_as_new",
                        ...
                        .buttonStyle(.bwr(.primary, size: .small))
                }
                if let onRevert {
                    Button(basedOn == nil
...
struct ProfileDetailCard: View {
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 8) {
                    Text(name)
                        .font(.bwrText(16, weight: .semibold))
                        .foregroundStyle(theme.text)
    ...
    private var sections: some View {
        ...
        // `MODEL_SPECIFIC_PROFILE_FIELDS` in `bwr/model_profiles.py`.
    ...
    private func specprefillLabel(_ s: [String: AnyCodable], on: Bool) -> String {
        ...
        // `bwr/model_settings.py:62`); render as a percent so the chip
        // reads naturally.
                ...
                HStack(spacing: 8) {
                    Toggle(isOn: Binding(
                        ...
                        Text(String(localized: "profile.detail.expose_as_model",
                            ...
                            .font(.bwrText(11.5, weight: .medium))
                            .foregroundStyle(theme.textSecondary)
                    }
                    .toggleStyle(.switch)
        ...
        VStack(alignment: .leading, spacing: 5) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Text(String(format: fmt, value))
                    .font(.bwrMono(12.5, weight: .semibold))
                    .foregroundStyle(theme.text)
            }
    ...
    private func stat(label: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(label)
                ...
                .foregroundStyle(theme.textSecondary)
            Text(value)
                .font(.bwrMono(14, weight: .semibold))
                .foregroundStyle(theme.text)
                .lineLimit(1)
                .truncationMode(.tail)
        }
