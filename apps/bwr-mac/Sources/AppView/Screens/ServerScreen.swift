            ...
            ListGroup {
                Row(
                    ...
                    sublabel: String(localized: "server.row.base_path.sub",
                                     defaultValue: "BWR_BASE_PATH. Files move and the server restarts when this changes.",
    ...
    private func modelDirRow(index: Int) -> some View {
        HStack(spacing: 7) {
            Text(index == 0
                ...
                .font(.bwrMono(10, weight: .semibold))
...
/// Hero card shared between the Server and Status screens (bwr-screens.jsx
...
struct ServerHeroCard: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            // App logo — `AppLogo` asset ships the rounded bwr mark (same
            // artwork as the README's hero icon, light/dark variants).
            // Replaces the previous gradient squircle + "oM" placeholder
            // so the hero card on Server / Status reads as BigWhiteRabbit rather
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 10) {
                    Text(title)
                        .font(.bwrText(18, weight: .semibold))
                        .foregroundStyle(theme.text)
    ...
    private var title: String {
        ...
        return trimmed.isEmpty ? "BigWhiteRabbit" : "BigWhiteRabbit \(trimmed)"
    }
}
...
private struct ServerAdvancedSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // doesn't match the rest of the screen (uses SF Pro vs bwrText,
                ...
                ListGroup {
                    ...
                    Row(
                        ...
                        TextInput(
                            ...
                            placeholder: "bwr.local, BigWhiteRabbit.lan",
                            mono: true,
                            width: .controlWide
                        )
                    }
