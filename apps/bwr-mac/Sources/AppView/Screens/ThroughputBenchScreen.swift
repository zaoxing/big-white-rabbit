...
// (bwr/admin/templates/dashboard/_bench.html + dashboard.js benchmark
...
private struct SingleResultsTable: View {
            ...
            FreeRow {
                HStack(spacing: 10) {
                    ForEach(Array(columnHeaders.enumerated()), id: \.offset) { _, h in
                        Text(h)
                            .font(.bwrText(10.5, weight: .semibold))
                            .foregroundStyle(theme.textTertiary)
                            .textCase(.uppercase)
                            .kerning(0.4)
                            .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func cell(_ text: String, mono: Bool = false) -> some View {
        Text(text)
            .font(mono ? .bwrMono(11.5) : .bwrText(11.5))
...
/// Renders the result of the post-bench upload to the public bwr.ai
/// leaderboard. The upload happens server-side automatically after every
/// run (bwr/admin/benchmark.py:_upload_to_bwr_ai); we just surface the
/// state. Three modes:
                ...
                FreeRow(isLast: true) {
                    HStack(spacing: 10) {
                        ...
                        Text(String(localized: "bench.throughput.upload.uploading",
                                    defaultValue: "Uploading to bwr.ai…",
        ...
        HStack(alignment: .top, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(String(localized: "bench.throughput.upload.skipped.title",
                    ...
                    .font(.bwrText(12, weight: .semibold))
                    .foregroundStyle(theme.text)
