...
struct UsageHistoryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let error {
                Text(error).font(.bwrText(12)).foregroundStyle(.secondary)
                    .padding(.horizontal, 18)
            }
            if disabled {
                HStack(spacing: 12) {
                    Text(String(localized: "status.usage.disabled",
                        ...
                        .font(.bwrText(12)).foregroundStyle(.secondary)
    ...
    private func tile(_ label: String, _ value: Int) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(label).font(.bwrText(11)).foregroundStyle(.secondary)
            Text(compact(value)).font(.bwrMono(20))
        }
    ...
    private func heatmap(_ days: [UsageHistoryDTO.UsageDayDTO]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(String(localized: "status.usage.heatmap.note",
                ...
                .font(.bwrText(11)).foregroundStyle(.secondary)
            HStack(spacing: 3) {
                Text("").frame(width: 78)
                ForEach(0..<24) { hour in
                    Text(hour % 3 == 0 ? String(format: "%02d", hour) : "")
                        .font(.bwrMono(9)).frame(maxWidth: .infinity)
                }
            }
            ScrollView {
                LazyVStack(spacing: 3) {
                    ForEach(days.reversed()) { day in
                        HStack(spacing: 3) {
                            Text(day.date).font(.bwrMono(10)).frame(width: 78, alignment: .leading)
                            ForEach(0..<24) { hour in
