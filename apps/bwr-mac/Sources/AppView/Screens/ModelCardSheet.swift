...
//   • HF       → BWRClient.getHFModelCard(repoId:)
//   • MS       → BWRClient.getMSModelCard(modelId:)
//
...
struct ModelCardSheet: View {
    ...
    private var loraBanner: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            Text(String(localized: "downloads.card.lora_warning",
                        defaultValue: "This is a LoRA adapter. It needs a compatible base model to run — downloading on its own won't load in BigWhiteRabbit.",
    ...
    private func load() async {
        ...
        do {
            ...
            state = .failed(error.bwrDescription)
        }
    }
