...
struct StatusScreen: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Same hero card the Server screen uses (bwr-screens.jsx:75
            ...
            ListGroup {
                ...
                Row(label: String(localized: "status.row.version",
                                  defaultValue: "BigWhiteRabbit Version",
                                  comment: "Row label for the running BigWhiteRabbit version"),
                    isLast: true) {
                    Text(vm.versionText)
                        .font(.bwrMono(12))
                ...
                HStack(spacing: 8) {
                    ...
                    if hotTierEnabled {
                        Button(String(localized: "status.runtime_cache.clear_hot",
                            ...
                            .buttonStyle(.bwr(.destructive, size: .small))
...
/// `bwr/admin/static/js/dashboard.js` — 1024-scaled, one decimal place.
...
private struct StatTile: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(value)
                .font(.bwrText(22, weight: .semibold))
                .kerning(-0.5)
    ...
    private var primaryLine: some View {
        ...
        case .available(let upd), .ready(let upd):
            Text(String(localized: "status.updates.available_primary",
                        defaultValue: "BigWhiteRabbit \(upd.version) is available",
        ...
        case .idle:
            Text(updates.lastError == nil
                 ? String(localized: "status.updates.up_to_date_primary",
                          defaultValue: "BigWhiteRabbit is up to date",
