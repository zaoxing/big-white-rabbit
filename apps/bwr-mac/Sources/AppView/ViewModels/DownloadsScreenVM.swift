...
final class DownloadsScreenVM {
    ...
    @ObservationIgnored
    private weak var client: BWRClient?
    @ObservationIgnored
    ...
    func start(client: BWRClient) async {
        self.client = client
        pollTask?.cancel()
    ...
    private func loadActiveRecommendedIfNeeded(client: BWRClient) async {
        switch source {
        case .hf:
    ...
    private func refreshMirrors(client: BWRClient) async {
    ...
    private func refreshMsAvailability(client: BWRClient) async {
        do {
    ...
    func saveMirror(client: BWRClient) {
    ...
    func resetMirror(client: BWRClient) {
        mirrorDraft = ""
        saveMirror(client: client)
    }

    func saveMsMirror(client: BWRClient) {
    ...
    func resetMsMirror(client: BWRClient) {
        msMirrorDraft = ""
    ...
    func updateSearch(query rawQuery: String, client: BWRClient) {
    ...
    func updateMsSearch(query rawQuery: String, client: BWRClient) {
        let q = rawQuery.trimmingCharacters(in: .whitespacesAndNewlines)
        msSearchTask?.cancel()
        if q.isEmpty {
            msSearchResults = []
    ...
    func startDownload(repo: String? = nil, client: BWRClient) {
    ...
    func cancel(taskId: String, client: BWRClient) {
        let activeSource = source
        Task { [weak self] in
            do {
                switch activeSource {
    ...
    func retry(taskId: String, client: BWRClient) {
        let activeSource = source
        Task { [weak self] in
            do {
                switch activeSource {
    ...
    func remove(taskId: String, client: BWRClient) {
        let activeSource = source
        Task { [weak self] in
            do {
                switch activeSource {
    ...
    func loadRecommended(client: BWRClient) async {
