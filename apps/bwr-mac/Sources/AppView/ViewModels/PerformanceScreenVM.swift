...
final class PerformanceScreenVM {
    ...
    func save(client: BWRClient) async {
    ...
    var memoryGuardBreakdown: String? {
        ...
        let bwrGB = Double(sys.bwrPhysFootprintBytes ?? 0) / Self.bytesPerGB
        let pct = Int((ratio * 100).rounded())
        let (ceiling, kernelBinds) = clamp(bwrGB + freeGB + inactiveGB + reclaimGB)
        if kernelBinds {
    ...
    /// Red warning when the effective Metal cap sits below what BigWhiteRabbit asked
    ...
    var wiredLimitWarningText: String? {
        ...
        let requested = sys.bwrWiredLimitRequestBytes ?? 0
        ...
        return String(localized: "performance.memory.wired_limit_warning",
                      defaultValue: "Metal caps BigWhiteRabbit at \(kernelGB) GB (kernel iogpu.wired_limit_mb). Raise it in Terminal:",
                      comment: "Warning shown when the kernel Metal wired limit is below the memory ceiling BigWhiteRabbit requested")
    }

    /// The sysctl command paired with `wiredLimitWarningText`.
    var wiredLimitCommand: String {
        let requested = Double(systemInfo?.bwrWiredLimitRequestBytes ?? 0)
