...
final class IntegrationsScreenVM {
    ...
    /// Composed `bwr launch claude` command. Claude tier selections are
    ...
    /// vars, local form sets them to point at the BigWhiteRabbit server.
    ...
    func load(client: BWRClient) async {
        do {
            ...
            // default `bwr` prefix and 127.0.0.1:1919.
    ...
    private var cliCommandPrefix: String {
        cliPrefix == "bwr" ? cliPrefix : Self.shellQuote(cliPrefix)
    }
