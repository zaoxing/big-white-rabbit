...
final class ThroughputBenchScreenVM {
    ...
    /// request. Mirrors `_FEATURE_FLAG_SPECS` in bwr/admin/benchmark.py.
    ...
    func runBenchmark(client: BWRClient) {
    ...
    func cancelBenchmark(client: BWRClient) {
        ...
        Task { [weak self] in
            do {
                ...
                await MainActor.run { self?.lastError = error.bwrDescription }
            }
            await MainActor.run {
                self?.running = false
    ...
    private func pollResults(client: BWRClient) {
        pollTask?.cancel()
        ...
        pollTask = Task { [weak self] in
            while !Task.isCancelled {
                ...
                do {
                    ...
                    // (benchmark.py:_upload_to_bwr_ai) and surfaces it
