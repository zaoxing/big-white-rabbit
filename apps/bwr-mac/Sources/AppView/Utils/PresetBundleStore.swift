...
//
//   1. Disk cache (~/Library/Application Support/BigWhiteRabbit/preset_cache.json)
//   2. Bundled fixture (Contents/Resources/bwr/admin/static/bwr_preset.json)
//      — staged into the .app by Scripts/build.sh from the bwr package.
//   3. Remote refresh via POST /admin/api/presets/refresh (bwr.ai proxy).
//
...
// retired (see Phase 1 — bwr/template/default_global_profile.json
...
final class PresetBundleStore {
    ...
    /// HTML's `localStorage.bwr_preset_cache`.
    ...
    func refresh(client: BWRClient) async {
        ...
        do {
            ...
            self.lastError = (error as? BWRClientError)
    ...
    /// `Contents/Resources/bwr/admin/static/bwr_preset.json`. The file
    /// is staged into the app by `Scripts/build.sh` (bwr package rsync),
    ...
    private static func loadFixture() -> PresetBundleDTO? {
        ...
        let url = resources
            ...
            .appendingPathComponent("bwr_preset.json")
        guard FileManager.default.fileExists(atPath: url.path),
              let data = try? Data(contentsOf: url) else { return nil }
        return try? JSONDecoder().decode(PresetBundleDTO.self, from: data)
    }
