...
final class WelcomeWindowController: NSObject, NSWindowDelegate {
    static let willCloseNotification = Notification.Name("BWRWelcomeWillClose")
    ...
    func show() {
        ...
        win.title = String(localized: "welcome.window.title",
                           defaultValue: "Welcome to BigWhiteRabbit",
...
final class WelcomeViewModel: ObservableObject {
    ...
    func browseBaseDirectory() {
        ...
        panel.message = String(localized: "welcome.browse.base_message",
                               defaultValue: "Choose a parent folder. An .bwr directory will be created inside it.",
                               comment: "NSOpenPanel message when picking the Base Directory in Welcome wizard")
        if panel.runModal() == .OK, let url = panel.url {
            basePath = url.appendingPathComponent(".bwr", isDirectory: true).path
        }
    }
    ...
    func startServer() async -> Bool {
        ...
        // When the user kept the default ~/.bwr, clear every override.
        ...
        do {
            ...
            case .portConflict(let conflict):
                lastError = conflict.isBWR
                    ? String(localized: "welcome.error.port_in_use_bwr",
                             defaultValue: "Port \(String(config.port)) is already in use (BigWhiteRabbit server already running).",
                             comment: "Welcome wizard: bind() failed because another BigWhiteRabbit instance owns the port")
    ...
    private func setupServerApiKey(client: BWRClient, key: String) async -> Bool {
        // Try setup-api-key (fresh install). When the server already has a
        // key set, the endpoint returns 400 — we swallow that and let
        // `BWRClient`'s 401 auto-login handle the next authenticated call.
...
struct WelcomeView: View {
    ...
    var body: some View {
        let theme = scheme == .dark ? BWRTheme.dark : BWRTheme.light
        ZStack {
            WelcomeBackdrop()
                .ignoresSafeArea()
        ...
        VStack(spacing: 24) {
            ...
            VStack(spacing: 14) {
                Text(String(localized: "welcome.header.title",
                            defaultValue: "BigWhiteRabbit",
                            comment: "Main heading shown on the Welcome wizard"))
                    .font(.bwrDisplay(48, weight: .semibold))
                ...
                Text(String(localized: "welcome.header.subtitle",
                    ...
                    .font(.bwrDisplay(25, weight: .semibold))
                    .foregroundStyle(WelcomeStyle.text)
                    .multilineTextAlignment(.center)
        ...
        VStack(spacing: 22) {
            ...
            VStack(spacing: 6) {
                Text(String(localized: "welcome.setup.title",
                    ...
                    .font(.bwrDisplay(24, weight: .semibold))
                    .foregroundStyle(WelcomeStyle.text)
            ...
            VStack(alignment: .leading, spacing: 18) {
                WelcomeNotice(
                    ...
                    text: String(localized: "welcome.setup.local.body",
                                 defaultValue: "BigWhiteRabbit binds to 127.0.0.1 on first run, so clients on this Mac can use it without exposing the server to your network.",
                ...
                VStack(spacing: 0) {
                    ...
                    SettingRow(
                        ...
                        HStack(spacing: 8) {
                            ...
                            .buttonStyle(.bwr(.normal, size: .small))
                    ...
                    SettingRow(
                        ...
                        HStack(spacing: 0) {
                            TextInput("welcome.api_key.placeholder", text: $vm.apiKey, placeholder: "sk-bwr-…", isSecure: !keyVisible, mono: true, width: .controlMedium)
                            Button {
                                keyVisible.toggle()
                            } label: {
                ...
                Text(String(localized: "welcome.hint.settings_path",
                            defaultValue: "Settings are stored in ~/.bwr/settings.json.",
        ...
        VStack(spacing: 24) {
            ...
            VStack(spacing: 10) {
                Text(String(localized: "welcome.complete.title",
                    ...
                    .font(.bwrDisplay(30, weight: .semibold))
                    .foregroundStyle(WelcomeStyle.text)
                Text(String(localized: "welcome.complete.description",
                            defaultValue: "BigWhiteRabbit is running locally at http://127.0.0.1:\(vm.portText). Open the web dashboard to download your first model, manage settings, and connect your coding tools.",
...
private struct WelcomeIcon: View {
    ...
    var body: some View {
        Image("AppLogo")
            ...
            .accessibilityLabel("BigWhiteRabbit")
    }
}
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text(title)
                    .font(.bwrText(13, weight: .semibold))
                    .foregroundStyle(WelcomeStyle.text)
                Text(text)
                    .font(.bwrText(12))
                    .foregroundStyle(WelcomeStyle.muted)
