...
private struct BWRThemeKey: EnvironmentKey {
    static let defaultValue: BWRTheme = .light
}

extension EnvironmentValues {
    var bwrTheme: BWRTheme {
        get { self[BWRThemeKey.self] }
        set { self[BWRThemeKey.self] = newValue }
    }
}

private struct BWRThemeBinder: ViewModifier {
    @Environment(\.colorScheme) private var scheme
    func body(content: Content) -> some View {
        content.environment(\.bwrTheme, scheme == .dark ? .dark : .light)
    }
}

extension View {
    /// Resolves `\.bwrTheme` from the current `\.colorScheme`. Apply once at
    /// the AppView shell (PR 6) so every descendant primitive reads the right
    /// palette without explicit prop-drilling.
    func bwrThemed() -> some View { modifier(BWRThemeBinder()) }
}
...
extension Font {
    ...
    static func bwrText(_ size: CGFloat, weight: Font.Weight = .regular) -> Font {
        .system(size: size, weight: weight)
    }
    /// Display weight for headlines (size auto-promotes on macOS at ≥ 20pt).
    static func bwrDisplay(_ size: CGFloat, weight: Font.Weight = .semibold) -> Font {
        .system(size: size, weight: weight)
    }
    /// Monospaced (SF Mono fallback chain).
    static func bwrMono(_ size: CGFloat, weight: Font.Weight = .regular) -> Font {
