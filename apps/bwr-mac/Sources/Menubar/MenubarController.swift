...
//   • Chat with BigWhiteRabbit    (enabled when running — opens /admin/chat in browser)
//   • Settings…         (Cmd-, — opens the SwiftUI AppView window via the
//                        openAppView callback; available even when stopped)
//   • About BigWhiteRabbit
//   • Quit BigWhiteRabbit       (Cmd-Q)
//
...
final class MenubarController: NSObject {
    ...
    static let restoreIconRequestNotification = Notification.Name("BWRMenubarRestoreIconRequest")
    ...
    init(
        ...
        let fallback = NSImage(
            ...
            accessibilityDescription: "BigWhiteRabbit"
        )
        ...
        statusItem.button?.setAccessibilityLabel("BigWhiteRabbit")
        statusItem.button?.toolTip = "BigWhiteRabbit"
    ...
    private func buildMenu() {
        ...
        chatItem = item(String(localized: "menubar.item.chat",
                               defaultValue: "Chat with BigWhiteRabbit",
        ...
        let about = item(String(localized: "menubar.item.about",
                                defaultValue: "About BigWhiteRabbit",
        ...
        let quit = item(String(localized: "menubar.item.quit",
                               defaultValue: "Quit BigWhiteRabbit",
    ...
    private func refreshUpdateMenuItem() {
        ...
        case .available(let info), .ready(let info):
            ...
            updateItem.title = String(localized: "menubar.item.install_update_version",
                                      defaultValue: "Install BigWhiteRabbit \(info.version)…",
        ...
        case .stopped:
            ...
            return (
                String(localized: "menubar.header.stopped",
                       defaultValue: "BigWhiteRabbit stopped",
    ...
    private func buildSystemStatsSubmenu() {
        ...
        let cpu = NSHostingView(rootView: AnyView(
            CPUStatsPanel(snapshot: snapshot, refreshInterval: interval).bwrThemed()
        ))
        let gpu = NSHostingView(rootView: AnyView(
            GPUStatsPanel(snapshot: snapshot, refreshInterval: interval).bwrThemed()
        ))
        let memory = NSHostingView(rootView: AnyView(
            MemoryStatsPanel(snapshot: snapshot).bwrThemed()
        ))
        cpuPanelHost = cpu
        gpuPanelHost = gpu
        ...
        if session == nil && alltime == nil {
            statsSubmenu.addItem(disabled(statsPoller == nil
                                          ? String(localized: "menubar.stats.no_api_key",
                                                   defaultValue: "Set BWR_API_KEY to enable stats",
    ...
    private func recreateStatusItem() -> NSStatusItem {
        ...
        newStatusItem.button?.setAccessibilityLabel("BigWhiteRabbit")
        newStatusItem.button?.toolTip = "BigWhiteRabbit"
    ...
    @objc private func serverPortConflict(_ note: Notification) {
        ...
        let key = "\(conflict.pid.map { String($0) } ?? "unknown"):\(conflict.isBWR)"
    ...
    @objc private func startServer() {
        ...
        do {
            ...
            NSLog("BigWhiteRabbit: start failed — \(error)")
        }
    }
    ...
    @objc private func forceRestartServer() {
        ...
        Task { @MainActor in
            do {
                ...
                NSLog("BigWhiteRabbit: force-restart failed — \(error)")
            }
        }
    }
    ...
    private func presentPortConflictAlert(_ conflict: PortConflict) {
        ...
        alert.informativeText = conflict.isBWR
            ? String(localized: "menubar.alert.port_in_use.bwr",
                     defaultValue: "Another BigWhiteRabbit server is already running on this port (\(pidStr)). Stop it before starting a new instance, or change the port in Settings.",
                     comment: "Port-conflict alert body when the conflicting process is another BigWhiteRabbit instance")
    ...
    private func presentServerFailureAlert(message: String, logURL: URL) {
        ...
        alert.messageText = String(localized: "menubar.alert.server_failed.title",
                                   defaultValue: "BigWhiteRabbit Server Failed to Start",
        ...
        alert.addButton(withTitle: String(localized: "menubar.alert.open_settings",
                                          ...
                                          comment: "Button that opens the BigWhiteRabbit settings window"))
...
extension MenubarController {
    ...
    static func accessFailureHint(message: String, logTail: String?) -> String? {
        ...
        return String(localized: "menubar.alert.server_failed.access_hint",
                      defaultValue: "Check that the configured model directory is mounted and readable. If it is on an external or protected location, grant BigWhiteRabbit access in macOS Privacy & Security settings.",
