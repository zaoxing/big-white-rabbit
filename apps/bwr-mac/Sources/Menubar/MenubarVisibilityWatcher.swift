...
// `bwr diagnose menubar` both point at menubar.log, so the probe has to
...
final class MenubarVisibilityWatcher {
    ...
    private func showHiddenAlert() {
        ...
        let alert = NSAlert()
        alert.messageText = "BigWhiteRabbit Menubar Icon Hidden"

        if MenubarIconRecovery.isTahoeOrNewer {
            alert.informativeText = """
            The BigWhiteRabbit menubar icon isn't showing up.
            ...
            BigWhiteRabbit and restart ControlCenter. It needs Full Disk Access.

            You can also enable BigWhiteRabbit manually in System Settings > Menu Bar, \
            ...
            third-party menubar apps. Try quitting and relaunching BigWhiteRabbit, \
...
/// `~/Library/Application Support/BigWhiteRabbit/logs/menubar.log`. The pre-Swift app
/// wrote this file and both the hidden-icon alert and `bwr diagnose
...
enum MenubarLog {
    ...
    static var url = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Application Support/BigWhiteRabbit/logs/menubar.log")
    ...
    private static func trimIfNeeded() {
        ...
        // file stays complete and parseable by `bwr diagnose menubar`.
...
enum MenubarIconRecovery {
    ...
    static func restore(rebuild: () -> Void) {
        ...
        guard isTahoeOrNewer else {
            ...
            showRestoreResultAlert(
                message: """
                BigWhiteRabbit rebuilt its menu bar item.

                macOS before Tahoe doesn't offer a System Settings toggle for \
                third-party menubar apps. If the icon still doesn't appear, \
                quit and relaunch BigWhiteRabbit, and check menubar manager tools like \
        ...
        showRestoreResultAlert(
            message: "BigWhiteRabbit rebuilt its menu bar item.\n\n\(result.message)",
            offerSettings: true
        )
    }
    ...
    private static func fixStatusKitPermission() -> AutoFixOutcome {
        ...
        let primaryBundleID = Bundle.main.bundleIdentifier ?? "app.bwr"
        let targetBundleIDs = Set([primaryBundleID, "app.bwr", "com.bwr.app"])
        ...
        for var entry in entries {
            ...
            let referencesBwr = locationMatches || menuMatches

            guard referencesBwr else {
                normalizedEntries.append(entry)
                continue
            }
            ...
            guard let bundleID = locationBundleID, targetBundleIDs.contains(bundleID) else {
                ...
                // menuItemLocations=BigWhiteRabbit. ControlCenter may keep those around
                // after bundle-id changes, but they do not back the BigWhiteRabbit toggle.
                changed = true
                continue
            }
        ...
        if !changed {
            ...
            return AutoFixOutcome(
                success: true,
                message: """
                BigWhiteRabbit is already approved in StatusKit (\(knownIDs)). If the \
        ...
        if !restartControlCenter() {
            return AutoFixOutcome(
                success: true,
                message: """
                StatusKit was updated but BigWhiteRabbit couldn't restart ControlCenter. \
    ...
    private static func backupStatusKitPlist() -> URL? {
        ...
        let backupDirectory = fileManager.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/BigWhiteRabbit/backups")
        let formatter = DateFormatter()
    ...
    private static func writeStatusKitPlist(_ data: Data, backup: URL?) throws {
        ...
        let temporaryURL = statusKitPlistURL
            ...
            .appendingPathComponent(statusKitPlistURL.lastPathComponent + ".bwr-tmp")
    ...
    private static func showStatusKitAccessDeniedAlert() {
        ...
        Enable BigWhiteRabbit in System Settings > Privacy & Security > Full Disk \
        Access, then run Auto-Fix again. You can also turn BigWhiteRabbit back on \
