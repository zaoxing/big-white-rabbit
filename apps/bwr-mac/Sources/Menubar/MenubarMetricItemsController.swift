...
final class MenubarMetricItemsController: NSObject, NSPopoverDelegate {
    ...
    func sync() {
        for spec in Self.specs {
            ...
            if entries[spec.kind]?.lastSignature != signature {
                ...
                button.toolTip = "BigWhiteRabbit · \(spec.kind.displayName)"
    ...
    /// when the main BigWhiteRabbit menu opens so dropdowns never stack.
    ...
    private func makeEntry(for spec: ItemSpec) -> ItemEntry {
        ...
        item.autosaveName = "bwr.metric.\(spec.kind.rawValue)"
        ...
        if let button = item.button {
            ...
            button.setAccessibilityLabel("BigWhiteRabbit \(spec.tag)")
        }
