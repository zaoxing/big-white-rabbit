...
//
// Flow: download .dmg → hdiutil attach → copy the inner BigWhiteRabbit.app next to
// the running bundle as `.BigWhiteRabbit-update.app` → hdiutil detach → on
...
final class AppUpdater {
    enum UpdateError: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case .notWritable(let path):
                return "Cannot write to \(path). Move BigWhiteRabbit.app to a writable location and try again."
            ...
            case .appNotFoundInVolume: return "BigWhiteRabbit.app not found inside the downloaded DMG"
    ...
    private func run(app: URL) async {
        ...
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bwr-update-\(UUID().uuidString)")
        do {
        ...
        let dmgPath = tmpDir.appendingPathComponent("BigWhiteRabbit-\(version).dmg")
    ...
    private func findAppInVolume(_ mountPoint: URL) throws -> URL {
        let preferred = mountPoint.appendingPathComponent("BigWhiteRabbit.app")
    ...
    static func performSwapAndRelaunch() -> Bool {
        ...
        guard let executable = Bundle.main.executableURL else {
            NSLog("BigWhiteRabbit: failed to locate executable for updater worker")
            return false
        }
        do {
            ...
            NSLog("BigWhiteRabbit: failed to start updater worker: %@", error.localizedDescription)
            return false
        }
        return true
    }
