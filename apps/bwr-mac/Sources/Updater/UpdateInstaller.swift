...
enum UpdateInstaller {
    static let workerModeArgument = "--bwr-update-worker"
    static let stagedAppName = ".BigWhiteRabbit-update.app"
    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .parentExitTimedOut(let pid):
                return "Timed out waiting for BigWhiteRabbit process \(pid) to exit"
            ...
            case .relaunchFailed(let message):
                return "Could not relaunch BigWhiteRabbit: \(message)"
            }
        }
    }
    ...
    static func submitWorker(
        ...
        let label = "app.bwr.updater.\(identifier)"
    ...
    static func cleanupLaunchAgents(in jobsDirectory: URL) {
        ...
        for plistURL in entries where plistURL.pathExtension == "plist" {
            if let data = try? Data(contentsOf: plistURL),
               ...
               label.hasPrefix("app.bwr.updater.") {
                _ = try? runProcess(
                    "/bin/launchctl",
    ...
    static func runWorker(_ request: WorkerRequest) -> Int32 {
        do {
            ...
            NSLog("BigWhiteRabbit updater: %@", error.localizedDescription)
            try? relaunch(request.liveApp)
            return EXIT_FAILURE
        }
    }
    ...
    private static func clearQuarantine(at app: URL) {
        ...
        NSLog(
            "BigWhiteRabbit updater: could not clear quarantine: %@",
            result.stderr.isEmpty ? result.stdout : result.stderr
        )
    }
