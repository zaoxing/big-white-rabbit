...
// `~/Library/Application Support/BigWhiteRabbit/update-prefs.json` so they survive
// a relaunch.
//
...
final class UpdateController {
    static let stateDidChangeNotification = Notification.Name("BWRUpdateControllerStateDidChange")
    ...
    private func runCheck(userInitiated: Bool) async {
        do {
            ...
            NSLog("BigWhiteRabbit: update check failed — %@", String(describing: error))
            await MainActor.run {
                if userInitiated {
