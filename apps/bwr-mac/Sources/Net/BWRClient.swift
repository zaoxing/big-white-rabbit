...
enum BWRClientError: Error, CustomStringConvertible {
    case invalidURL
...
extension Error {
    ...
    /// BWRClientError formats via its `description` (HTTP status + body,
    /// authentication state, etc.); other errors fall back to
    /// `localizedDescription`. Replaces the per-VM `describe(_:)` helper.
    var bwrDescription: String {
        if let bwr = self as? BWRClientError { return String(describing: bwr) }
        return localizedDescription
    }
}

@MainActor
final class BWRClient: ObservableObject {
    ...
    /// Force-refresh the preset bundle from bwr.ai (via the server proxy
    ...
    // pipeline implemented by `bwr/admin/ms_downloader.py`. Task / model
    ...
    private func request<T: Decodable>(
        ...
        guard let url = components.url else { throw BWRClientError.invalidURL }
        ...
        guard let http = resp as? HTTPURLResponse else { throw BWRClientError.invalidResponse }

        if http.statusCode == 401, !isRetry {
            guard let key = apiKey, !key.isEmpty else {
                throw BWRClientError.unauthenticated
            }
        ...
        guard 200..<300 ~= http.statusCode else {
            ...
            throw BWRClientError.http(status: http.statusCode, body: bodyStr)
        }
