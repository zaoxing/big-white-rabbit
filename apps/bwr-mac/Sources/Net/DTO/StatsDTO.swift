...
struct StatsDTO: Codable, Equatable, Sendable {
    ...
    let apiKey: String?
    /// Absolute CLI invocation prefix used for `bwr launch <tool>` strings.
    /// `nil` on older servers; callers fall back to the bare `"bwr"` token
    ...
    /// Mirrors `_build_runtime_cache_observability` in `bwr/admin/routes.py`.
