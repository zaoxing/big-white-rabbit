...
struct GlobalSettingsDTO: Codable, Equatable, Sendable {
    ...
    struct SystemInfo: Codable, Equatable, Sendable {
        ...
        /// BigWhiteRabbit process phys_footprint at fetch time.
        let bwrPhysFootprintBytes: Int64?
        ...
        /// What BigWhiteRabbit asked Metal to allow at start (static ceiling clamped
        /// below physical RAM). Kernel cap below this = red warning.
        let bwrWiredLimitRequestBytes: Int64?
    }

    /// Mirrors `bwr.settings.HuggingFaceSettings`. Empty string means
    ...
    /// Mirrors `bwr.settings.SamplingSettings`. The full server surface
    ...
    /// Mirrors `bwr.settings.MCPSettings`. The server stores a single path to
    ...
    /// Mirrors `bwr.settings.UsageSettings`. `usage_history` switches the
    ...
    /// Mirrors `bwr.settings.ModelScopeSettings`. Empty string means
    /// "use the default" (modelscope.cn). Patched via `ms_endpoint`.
    struct ModelScopeDTO: Codable, Equatable, Sendable {
        let endpoint: String
    }

    /// Mirrors `bwr.settings.NetworkSettings`. All four fields are simple
...
struct GlobalSettingsPatch: Encodable, Equatable, Sendable {
    ...
    // see `bwr/admin/routes.py:229-234`. They patch in-place; non-nil fields
    ...
    /// place via `bwr/admin/routes.py:2804`.
    ...
    // applies live wherever possible — see `bwr/admin/routes.py` for the
