...
//   1. BWR_BASE_PATH env var. macOS Finder/Dock launches inherit only the
...
//   2. ~/Library/Application Support/BigWhiteRabbit/base-path — a one-line
...
struct AppConfig: Sendable, Equatable, Codable {
    ...
    var apiKey: String?
    /// Always `BWR_BASE_PATH` if set, else `~/.bwr`. Set at load() time
    ...
    /// has no `BWR_BASE_PATH`. The single field it carries is the path
    ...
    /// "reset to ~/.bwr default" flow. Callers should compare against
    ...
    static func defaultBasePath() -> String {
        ...
        return home.appendingPathComponent(".bwr", isDirectory: true).path
    }
    ...
    static func appSupportURL() -> URL {
        ...
        let dir = base.appendingPathComponent("BigWhiteRabbit", isDirectory: true)
    ...
    static func load() -> AppConfig {
        ...
        // via currentBasePath().
        let env = ProcessInfo.processInfo.environment
        if let h = env["BWR_HOST"], !h.isEmpty { c.bindAddress = h }
        if let pStr = env["BWR_PORT"], let p = Int(pStr) { c.port = p }
        if let k = env["BWR_API_KEY"], !k.isEmpty { c.apiKey = k }
        return c
    }
