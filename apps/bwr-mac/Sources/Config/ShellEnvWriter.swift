...
// `~/.bwr/bin/bwr`, then tries to expose it through a safe public symlink.
// Shell rc edits are kept behind an explicit user prompt only.

import Foundation

enum ShellEnvWriter {
    static let variableName = "BWR_BASE_PATH"
    ...
    private static let cliShimBeginMarker = "# BigWhiteRabbit: CLI shim path begin"
    private static let cliShimEndMarker = "# BigWhiteRabbit: CLI shim path end"
    private static let publicBinCandidates = [
        "/opt/homebrew/bin",
        "/usr/local/bin",
    ]

    /// Install/update `~/.bwr/bin/bwr` so app-only installs still expose
    ...
    static func ensureCLIShim(appBundleURL: URL = Bundle.main.bundleURL) throws -> CLISetupResult {
        ...
        let shimURL = shimDir.appendingPathComponent("bwr")
        try writeLauncherShim(at: shimURL, forwardingTo: bundleCLI)

        // Another Mac's coordinator discovers this node by running
        // `~/.bwr/bin/bwr-cluster-python -c 'import bwr'` over SSH. The CLI
        // wrapper above cannot answer that — it hardcodes `-m bwr.cli` — so a
        ...
        if FileManager.default.isExecutableFile(atPath: clusterPython.path) {
            ...
            try? writeLauncherShim(
                at: shimDir.appendingPathComponent("bwr-cluster-python"),
            ...
            if let first = firstCLIPathInCurrentPath(),
               !isManagedCLI(path: first, shimURL: shimURL) {
                return .needsShellPathPrompt(
                    reason: "\(first.path) appears before the BigWhiteRabbit app-managed command on PATH."
                )
            }
            return .publicCommandReady(path: path.path)
    ...
    /// Write an executable `/bin/sh` shim that restores `BWR_BASE_PATH` from
    ...
    private static func writeLauncherShim(at shimURL: URL, forwardingTo target: URL) throws {
        let script = """
        #!/bin/sh
        BOOTSTRAP="$HOME/Library/Application Support/BigWhiteRabbit/base-path"
        if [ -r "$BOOTSTRAP" ]; then
    ...
    private static func ensurePublicSymlink(to shimURL: URL) -> PublicSymlinkResult {
        ...
        for dir in publicBinDirs() {
            ...
            let link = dir.appendingPathComponent("bwr")
            if fm.fileExists(atPath: link.path) {
                if isManagedCLI(path: link, shimURL: shimURL) {
                    return .installed(link)
                }
                reasons.append("\(link.path) already exists and is not managed by BigWhiteRabbit.")
                continue
            }
