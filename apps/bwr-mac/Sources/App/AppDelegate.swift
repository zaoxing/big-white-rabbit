...
// `Window` scene declared in BigWhiteRabbitApp.swift — we no longer build it
// manually here.
//
// Boot flow
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Quit / "Quit BigWhiteRabbit" from the application menu all route through
    ...
    /// `bwrapp://main` URL back to this exact app bundle. The Window scene
    /// in BigWhiteRabbitApp.swift handles the targeted external event.
    func presentAppView() {
        ...
        if let url = URL(string: "bwrapp://main") {
            // Target this bundle explicitly. A source build and an installed
            // release share the app.bwr identifier and bwrapp URL scheme;
            // resolving the URL globally can launch the other copy and leave
            // two BigWhiteRabbit processes in the Dock.
            ...
            NSWorkspace.shared.open(
                ...
                if let error {
                    NSLog("BigWhiteRabbit: failed to open main window: \(error)")
                }
            }
        }
    }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !isRunningUnitTests {
            do {
                ...
                NSLog("BigWhiteRabbit: CLI shim setup failed — \(error)")
            }
            startControlServer()
        }
        ...
        if AppConfig.hasExistingConfig {
            ...
            // file (`~/.bwr/settings.json` first, Library config.json
    ...
    private func promptForShellPathExport(reason: String) {
        let alert = NSAlert()
        alert.messageText = "Enable `bwr` in Terminal?"
        alert.informativeText = """
        BigWhiteRabbit could not create a public `bwr` command in /opt/homebrew/bin or /usr/local/bin.

        To make `bwr` available in new Terminal sessions, BigWhiteRabbit can add a small PATH block to your shell init file. This only happens if you choose Update Shell File.
        ...
        case .alertFirstButtonReturn:
            do {
                ...
                NSLog("BigWhiteRabbit: CLI shell path setup failed — \(error)")
            }
    ...
    private func bootstrapServer(config: AppConfig) {
        do {
            ...
            NSLog("BigWhiteRabbit: server bootstrap failed — \(error)")
        }
    }
    ...
    @objc private func windowDidBecomeMainNotification(_ notif: Notification) {
        ...
        // (the "BigWhiteRabbit" floating above the toolbar zone). The title string
    ...
    /// The single real-quit path is the menubar status item's "Quit BigWhiteRabbit",
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        // Same close path used by the SwiftUI Cmd-Q command in BigWhiteRabbitApp.swift.
    ...
    private func startControlServer() {
        ...
        do {
            ...
            NSLog("BigWhiteRabbit: app-control server failed to start — \(error)")
        }
    }
    ...
    func handleAppControl(_ command: AppControlServer.Command) async -> AppControlServer.Response {
        guard let server else {
            return .failure(
                ...
                state: .stopped,
                server: nil,
                message: "Managed server is unavailable. Complete the BigWhiteRabbit first-run setup in the app."
            )
        }
        ...
        case .stop:
            ...
            return .success(
                ...
                state: server.state,
                server: server,
                message: "BigWhiteRabbit stopped"
            )
