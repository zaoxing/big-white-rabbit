#!/usr/bin/env bash
# build.sh — produce a runnable BigWhiteRabbit.app for local manual testing.
#
# Side-by-side Swift bundle path: this builds `BigWhiteRabbit.app` alongside the
# legacy Python/PyObjC `BigWhiteRabbit.app` until the Swift app becomes the primary
# release channel.
# Pipeline:
...
#   4. embed the bwr package from the worktree and ad-hoc sign
#
...
#   1. $BWR_DONOR_APP — explicit override (e.g. /Applications/BigWhiteRabbit.app).
...
#   3. /Applications/BigWhiteRabbit.app — last-resort fallback when --no-rebuild-donor
#                               is set and no local export exists.
#
# Usage:
#   apps/bwr-mac/Scripts/build.sh                    # Release, auto-rebuild donor when stale
#   apps/bwr-mac/Scripts/build.sh debug              # Debug build instead
#   apps/bwr-mac/Scripts/build.sh swift              # rebuild Swift app only; reuse existing _export/
#   apps/bwr-mac/Scripts/build.sh swift debug        # Debug Swift app rebuild; reuse existing _export/
#   apps/bwr-mac/Scripts/build.sh swift-fast         # like swift, but skip embedded native signing
#   apps/bwr-mac/Scripts/build.sh release --bare     # skip Python embed
#                                                       (no server, just the
#                                                       AppView shell)
#   apps/bwr-mac/Scripts/build.sh release --with-custom-kernel
#                                                     # build and bundle optional
#                                                     # native custom kernels
#   apps/bwr-mac/Scripts/build.sh --rebuild-donor    # force venvstacks rebuild
#   apps/bwr-mac/Scripts/build.sh --no-rebuild-donor # never rebuild; use
#                                                       existing donor even if stale
#
# Env overrides:
#   BWR_DONOR_APP=/path/to/BigWhiteRabbit.app    # explicit donor (bypasses venvstacks)
#   BWR_EXPORT_DIR=/path/to/_export    # override the venvstacks export tree
#                                       (release builds set this per macOS target)
#   BWR_NEXT_OUT=/path/to/output_dir   # final stage location
#   PYTHON_BIN=/path/to/python3         # python used for venvstacks driver
#                                       (default: PATH lookup of python3)
#   BWR_WITH_CUSTOM_KERNEL=1           # same as --with-custom-kernel
#   BWR_CUSTOM_KERNEL_DEPLOYMENT_TARGET=15.0
...
BARE=0
WITH_CUSTOM_KERNEL="${BWR_WITH_CUSTOM_KERNEL:-0}"
...
CUSTOM_KERNEL_DIRS=(
    "$REPO_ROOT/bwr/custom_kernels/glm_moe_dsa"
    "$REPO_ROOT/bwr/custom_kernels/minimax_m3"
    "$REPO_ROOT/bwr/custom_kernels/qwen35_prefill"
)
CUSTOM_KERNEL_ABI_CHECKER="$SCRIPT_DIR/check_custom_kernel_python_abi.py"
# BWR_EXPORT_DIR overrides the venvstacks export tree we copy Python
...
LOCAL_EXPORT="${BWR_EXPORT_DIR:-$PACKAGING_DIR/_export}"

# BWR_DONOR_APP is "explicit" only when the user set it; the default
# (/Applications/BigWhiteRabbit.app) is treated as a fallback, not an override.
BWR_DONOR_APP_SET="${BWR_DONOR_APP+1}"
BWR_DONOR_APP="${BWR_DONOR_APP:-/Applications/BigWhiteRabbit.app}"
OUTPUT_DIR="${BWR_NEXT_OUT:-$PROJECT_DIR/build/Stage}"
...
_custom_kernel_deployment_target() {
    printf "%s\n" "${BWR_CUSTOM_KERNEL_DEPLOYMENT_TARGET:-${MACOSX_DEPLOYMENT_TARGET:-15.0}}"
}
...
_clean_custom_kernel_build_artifacts() {
    ...
    if [ -d "$REPO_ROOT/build" ]; then
        for ext_name in \
            "bwr.custom_kernels.glm_moe_dsa._ext" \
            "bwr.custom_kernels.minimax_m3._ext" \
            "bwr.custom_kernels.qwen35_prefill._ext"; do
...
for name in ("glm_moe_dsa", "minimax_m3", "qwen35_prefill"):
    ext_dir = pathlib.Path("bwr/custom_kernels") / name
...
_build_custom_kernels() {
    ...
    (
        ...
        export BWR_CUSTOM_KERNEL_DEPLOYMENT_TARGET="$deployment_target"
    ...
    [ -f "$REPO_ROOT/bwr/custom_kernels/glm_moe_dsa/bwr_glm_kernels.metallib" ] \
        || die "custom kernel build finished but GLM metallib is missing."
    [ -f "$REPO_ROOT/bwr/custom_kernels/minimax_m3/bwr_minimax_m3_kernels.metallib" ] \
        || die "custom kernel build finished but MiniMax M3 metallib is missing."
    [ -f "$REPO_ROOT/bwr/custom_kernels/qwen35_prefill/bwr_qwen35_prefill_kernels.metallib" ] \
    ...
    if _sdk_supports_nax; then
        [ -f "$REPO_ROOT/bwr/custom_kernels/qwen35_prefill/bwr_qwen35_prefill_kernels_nax.metallib" ] \
...
resolve_donor_layers() {
    ...
    # Explicit BWR_DONOR_APP override → use it, skip rebuild logic entirely.
    if [ -n "$BWR_DONOR_APP_SET" ]; then
        [ -d "$BWR_DONOR_APP" ] || die "BWR_DONOR_APP set but not found: $BWR_DONOR_APP"
        DONOR_LAYERS="$(_app_python_layers_path "$BWR_DONOR_APP")" \
            || die "BWR_DONOR_APP missing bundled Python layers: $BWR_DONOR_APP"
        DONOR_SOURCE="BWR_DONOR_APP=$BWR_DONOR_APP"
        return
    fi
    ...
    case "$REBUILD_DONOR" in
        ...
        never)
            ...
            elif [ -d "$BWR_DONOR_APP" ]; then
                DONOR_LAYERS="$(_app_python_layers_path "$BWR_DONOR_APP")" \
                    || die "Fallback donor missing bundled Python layers: $BWR_DONOR_APP"
                DONOR_SOURCE="$BWR_DONOR_APP (fallback, --no-rebuild-donor)"
            else
                die "No donor available: $LOCAL_EXPORT and $BWR_DONOR_APP both missing, --no-rebuild-donor prevents rebuild."
            fi
            ;;
        auto)
...
log "Building BigWhiteRabbit ($CONFIG)…"
mkdir -p "$BUILD_DIR"

log "Resolving Swift package dependencies…"
xcodebuild -resolvePackageDependencies \
    -project "$PROJECT_DIR/BigWhiteRabbit.xcodeproj" \
...
XCODE_APP="$BUILD_DIR/Build/Products/$CONFIG/BigWhiteRabbit.app"
...
mkdir -p "$OUTPUT_DIR"
STAGED_APP="$OUTPUT_DIR/BigWhiteRabbit.app"
...
if [ "$WITH_CUSTOM_KERNEL" = "1" ]; then
    _validate_packaged_custom_kernel_extensions \
        "$(_custom_kernel_donor_python)" "$RESOURCES_DIR/bwr/custom_kernels"
fi

log "Writing engine commit metadata..."
"$PYTHON_BIN" "$PACKAGING_DIR/build.py" --write-engine-commits "$RESOURCES_DIR/bwr" \
...
CLI_WRAPPER="$STAGED_APP/Contents/MacOS/bwr-cli"
cat > "$CLI_WRAPPER" <<'EOF'
#!/bin/sh
set -eu
...
#
# bwr-cli above hardcodes `-m bwr.cli`, so it can never answer the
# `-c 'import bwr'` probe a peer coordinator runs, nor host the `-c` scripts
...
# untouched — this is what ~/.bwr/bin/bwr-cluster-python points at, and what
# discover_remote_python_executable has always looked for (#2680).

log "Writing app-bundle cluster interpreter wrapper..."
CLUSTER_PYTHON_WRAPPER="$STAGED_APP/Contents/MacOS/bwr-cluster-python"
...
ICON_BUNDLE="$REPO_ROOT/apps/bwr-mac/Resources/AppIcon.icon"
XCASSETS_DIR="$REPO_ROOT/apps/bwr-mac/Resources/Assets.xcassets"
...
if [ "$SKIP_EMBEDDED_SIGN" -eq 1 ]; then
    ...
    ok "  + signed bwr-cli wrapper"
    codesign --force --sign - "$CLUSTER_PYTHON_WRAPPER" >/dev/null 2>&1
    ok "  + signed bwr-cluster-python wrapper"
fi
...
echo "  ~/Library/Application Support/BigWhiteRabbit/logs/server.log"
