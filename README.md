...
| `--mlx-batch` | Continuous batching (MLX) | Concurrent requests decode in one batched step. Output-identical. Aggregate throughput only **~1.16x** at 4 concurrent (MLX's quantized kernel does not amortise rows), but the last request's **first token arrives ~5x sooner** (11.9s → 2.3s). Latency feature, not a throughput one. Composes with `--mlx-prefix-cache`: a cached prompt skips prefill and still batches (399x TTFT measured while batched). Forces f16 KV: `mlx_kv_bits` is ignored with a warning, since mlx-lm has no batched `QuantizedKVCache` — that costs KV memory, not speed |
