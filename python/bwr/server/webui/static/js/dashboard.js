    ...
    // Glimmer DFlash drafter as "-assistant", which Big White Rabbit's name heuristics
    ...
    const THEME_STORAGE_KEY = 'bwr-chat-theme';
    const ENHANCED_READABILITY_KEY = 'bwr-enhanced-readability';
    ...
    function dashboard() {
        return {
            ...
            sortBy: localStorage.getItem('bwr_models_sort_by') || MODELS_SORT_DEFAULT.by,
            sortOrder: localStorage.getItem('bwr_models_sort_order') || MODELS_SORT_DEFAULT.order,
            modelSearch: '',
            // Manager tab (Browse Models > Local) sort + search state.
            managerSortBy: localStorage.getItem('bwr_manager_sort_by') || MANAGER_SORT_DEFAULT.by,
            managerSortOrder: localStorage.getItem('bwr_manager_sort_order') || MANAGER_SORT_DEFAULT.order,
            managerSearch: '',
            ...
            // oQ Uploader state
            uploadHfToken: localStorage.getItem('bwr-hf-upload-token') || '',
            uploadHfUsername: '',
            uploadHfOrgs: [],
            uploadHfNamespace: '',
            ...
            externalBaseUrl: localStorage.getItem('bwr_bench_external_base_url') || '',
            externalApiKey: localStorage.getItem('bwr_bench_external_api_key') || '',
            externalModel: localStorage.getItem('bwr_bench_external_model') || '',
            ...
            accExternalMaxTokens: localStorage.getItem('bwr_acc_external_max_tokens') || '',
            accRunning: false,
            accCurrentModel: '',
           ...
           async checkForUpdate() {
            ...
            async loadPresets() {
                ...
                const cached = localStorage.getItem('bwr_preset_cache');
                if (cached) {
                    try {
                ...
                try {
                    const r = await fetch('/admin/static/bwr_preset.json');
                    if (r.ok) {
                        const data = await r.json();
            ...
            async refreshPresets() {
                ...
                try {
                    ...
                    if (r.ok) {
                        const data = await r.json();
                        this.presets = data.presets || [];
                        localStorage.setItem('bwr_preset_cache', JSON.stringify(data));
                    } else if (r.status === 401) {
                        window.location.href = '/admin';
                    }
                } catch (e) {
            ...
            setScope(scope) {
                ...
                try { localStorage.setItem('bwr_profile_scope', scope); } catch (e) {}
            },
            ...
            async openModelSettings(model) {
                ...
                if (isDiffusion) {
                    ...
                    try {
                        const saved = localStorage.getItem('bwr_profile_scope');
                        ...
                        body: JSON.stringify((() => {
                            ...
                            const payload = {
                                ...
                                dflash_ssd_cache_max_bytes: this.modelSettings.dflash_enabled
                                    ...
                                    : 20 * (1024 ** 3),
                                // Long-context tuning. Empty → Big White Rabbit default.
            ...
            _launchCmd(tool) {
                const raw = this.stats.cli_prefix || 'bwr';
                const cli = raw === 'bwr' ? raw : this.shellQuote(raw);
            ...
            saveExternalEndpoint() {
                localStorage.setItem('bwr_bench_external_base_url', this.externalBaseUrl.trim());
                localStorage.setItem('bwr_bench_external_api_key', this.externalApiKey);
                localStorage.setItem('bwr_bench_external_model', this.externalModel.trim());
            },
            ...
            saveAccExternalMaxTokens() {
                localStorage.setItem('bwr_acc_external_max_tokens', this.accExternalMaxTokens.trim());
            },
            ...
            benchBuildText() {
                ...
                lines.push('Big White Rabbit - LLM inference, optimized for your Mac');
                lines.push('https://github.com/jundot/bwr');
            ...
            //   dynamic_ceiling = bwr_phys + free + inactive + active * ratio
            ...
            // iogpu.wired_limit_mb is lower than what Big White Rabbit asked Metal
            ...
            // working set) caps Big White Rabbit below its desired static ceiling.
            get memoryGuardShowWiredLimitWarning() {
                const sys = this.globalSettings.system || {};
                const kernelBytes = sys.iogpu_wired_limit_bytes || 0;
                const requestedBytes = sys.bwr_wired_limit_request_bytes || 0;
            ...
            get memoryGuardWiredLimitCommand() {
                if (!this.memoryGuardShowWiredLimitWarning) return '';
                const requested = this.globalSettings.system.bwr_wired_limit_request_bytes;
            ...
            get memoryGuardBreakdownHTML() {
                ...
                const omlxGB = (sys.bwr_phys_footprint_bytes || 0) / GB;
            ...
            toggleSort(column) {
                ...
                this.persistSort('bwr_models_sort_by', this.sortBy, 'bwr_models_sort_order', this.sortOrder);
            },
            ...
            resetSort() {
                ...
                try {
                    localStorage.removeItem('bwr_models_sort_by');
                    localStorage.removeItem('bwr_models_sort_order');
            ...
            toggleManagerSort(column) {
                ...
                this.persistSort('bwr_manager_sort_by', this.managerSortBy, 'bwr_manager_sort_order', this.managerSortOrder);
            },
            ...
            resetManagerSort() {
                ...
                try {
                    localStorage.removeItem('bwr_manager_sort_by');
                    localStorage.removeItem('bwr_manager_sort_order');
                } catch (e) { /* storage disabled */ }
            },
            ...
            async validateUploadToken() {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        localStorage.setItem('bwr-hf-upload-token', this.uploadHfToken);
