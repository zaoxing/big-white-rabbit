// Big White Rabbit Cluster v2 wizard — Alpine.js component backing
...
function clusterV2Wizard() {
    ...
    const CLUSTER_V2_STRINGS = {
        ...
        'cluster.v2.strategy.hint.auto':
            'Big White Rabbit picks the split that fits this model and your link',
    ...
    return {
        ...
        deploymentStatus(deployment = this.configuredDeployment()) {
            ...
            if (state === 'ready') {
                return {
                    ...
                    label: 'Ready',
                    detail:
                        'The distributed weights are resident and available through Big White Rabbit.',
                    tone: 'bg-green-50 border-green-200 text-green-700',
                    pulse: true,
                };
            }
            if (state === 'loading') {
                return {
                    ...
                    label: 'Loading',
                    detail:
                        'Big White Rabbit is loading and validating the model across your Macs.',
        ...
        async submitPairApproval(device) {
            ...
            try {
                ...
                if (error?.status === 404 || error?.status === 409) {
                    this.pairing.error =
                        'No join request from this Mac yet. On the other Mac, open its Big White Rabbit dashboard and press Pair first — then type its code here.';
        ...
        async submitManualPeer(options = {}) {
            ...
            try {
                ...
                if (result && result.verified) {
                    ...
                    this.notify(
                        'warning',
                        'No Big White Rabbit node answered at that address yet — it stays on the list while we keep trying.',
                    );
                }
                this.manualAddr = '';
            } catch (error) {
        ...
        checkRows() {
            ...
            // 3. Version parity (from the discovery snapshot, live).
            {
                ...
                rows.push({
                    key: 'version',
                    label: 'Matching Big White Rabbit versions',
                    ...
                    fix: 'Update the older Mac to the same Big White Rabbit build. App: it auto-updates. Brew: brew upgrade omlx. Source: pull the same commit on both Macs.',
                });
            }
            ...
            //    SSH + versions, so a red row would contradict "All clear".
            {
                ...
                let fix =
                    'macOS is blocking local-network beacons. System Settings → Privacy & Security → Local Network → allow Big White Rabbit, then restart Big White Rabbit. Pairing still works via Add by IP while this is amber.';
                if (health) {
        ...
        async generateCudaJoinCommand() {
            ...
            if (!controllerIp) {
                this.cudaJoin.error =
                    'Enter this Big White Rabbit Mac’s LAN IPv4 address first.';
                return;
            }
